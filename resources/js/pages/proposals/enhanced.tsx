...
export default function ProposalsPage() {
  ...
  const submitAsync = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/proposals/generate-async', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Requested-With': 'XMLHttpRequest',
      ...
      if (!res.ok) {
        throw new Error('Failed to queue proposal generation');
      }
      
      const json = await res.json();
      setAsyncMessage(json.message || 'Proposal queued successfully!');
    } catch {
      setError('Something went wrong. Please try again.');
    } finally {
              ...
              <CardContent className="space-y-4">
                <form onSubmit={submit} className="space-y-4">
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button type="submit" disabled={loading || asyncLoading} className="flex-1">
                      {loading ? (
                        <>
              ...
              <Card className="border-green-200 bg-green-50">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-green-800 font-medium">{asyncMessage}</span>
                  </div>
                  <p className="text-sm text-green-600 mt-1">
                    Check your <Link href="/proposals/history" className="underline">proposal history</Link> or monitor progress in the <Link href="/horizon" className="underline">queue dashboard</Link>.
                  </p>
                </CardContent>
              </Card>
            </div>
